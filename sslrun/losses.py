...
class LossStack(nn.Module):
    ...
    def __init__(self, model, loss_cfgs: list, ema_decay: float = 0.999,
                 ...
                 teacherless: bool = False, n_thoughts: int = 0,
                 thought_mode: str = "continuous", base_vocab: int = 0):
        super().__init__()
        ...
        # continuous thoughts (round 16, Coconut-style in pretraining):
        # insert BOT + m thought slots + EOT before the answer span. In
        # "continuous" mode slot j's input embedding is norm_f(last hidden)
        # of the previous position, generated by a sequential rollout that
        # gradients flow through; in "pause" mode the slots are m copies of
        # the BOT token (Goyal-style pause tokens — the parallelizable
        # control). Targets stay on real tokens only.
        assert thought_mode in ("continuous", "pause")
        assert not (n_thoughts and (teacherless or corrupt_p > 0))
        self.n_thoughts = n_thoughts
        self.thought_mode = thought_mode
        self.bot_id = base_vocab          # ids appended past the task vocab
        self.eot_id = base_vocab + 1
        self.model = model
    ...
    def _extend_with_thoughts(self, batch):
        """Insert BOT + m slots + EOT before the (contiguous) answer span,
        run the continuous rollout if needed, and return (batch', inject)."""
        tokens, tmask, vmask = (batch["tokens"], batch["target_mask"],
                                batch["valid_mask"])
        s_row = tmask.int().argmax(dim=1)
        assert bool((s_row == s_row[0]).all()), \
            "thoughts need a fixed answer start across the batch"
        s = int(s_row[0])
        B, T = tokens.shape
        m = self.n_thoughts
        D = self.model.cfg.d_model
        mid = tokens.new_full((B, m + 2), self.bot_id)
        mid[:, -1] = self.eot_id
        ext = torch.cat([tokens[:, :s], mid, tokens[:, s:]], dim=1)
        zmask = torch.zeros(B, m + 2, dtype=torch.bool, device=tokens.device)
        tmask_ext = torch.cat([tmask[:, :s], zmask, tmask[:, s:]], dim=1)
        vmask_ext = torch.cat([vmask[:, :s], ~zmask, vmask[:, s:]], dim=1)
        inject = None
        if self.thought_mode == "continuous":
            th = []      # generated thought embeddings, grads flow through
            for j in range(m + 1):
                L = s + 1 + j      # prefix + BOT + j filled slots
                inj = None
                if th:
                    ie = torch.zeros(B, L, D, device=tokens.device,
                                     dtype=th[0].dtype)
                    ie[:, s + 1:s + 1 + len(th)] = torch.stack(th, dim=1)
                    im = torch.zeros(B, L, dtype=torch.bool,
                                     device=tokens.device)
                    im[:, s + 1:s + 1 + len(th)] = True
                    inj = (im, ie)
                if j == m:
                    break            # rollout done; full pass comes next
                _, hid = self.model(ext[:, :L], inject=inj)
                th.append(self.model.norm_f(hid[-1][:, -1]))
            ie = torch.zeros(B, ext.size(1), D, device=tokens.device,
                             dtype=th[0].dtype)
            ie[:, s + 1:s + 1 + m] = torch.stack(th, dim=1)
            im = torch.zeros(B, ext.size(1), dtype=torch.bool,
                             device=tokens.device)
            im[:, s + 1:s + 1 + m] = True
            inject = (im, ie)
        batch = {**batch, "tokens": ext, "target_mask": tmask_ext,
                 "valid_mask": vmask_ext}
        return batch, inject

    def forward(self, batch):
        thought_inject = None
        if self.n_thoughts:
            batch, thought_inject = self._extend_with_thoughts(batch)
        drop = None
        if self.teacherless:
