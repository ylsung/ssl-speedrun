...
## Round 16 — continuous latent thoughts in pretraining (star graph)

User design (2026-07-15): Coconut-style thought slots at pretraining time.
Sequence becomes prefix + BOT + m latent slots + EOT + answer; in
`continuous` mode slot j's input embedding is norm_f(last hidden) of the
preceding position — generated by a sequential rollout (m+1 forwards,
gradients through the chain); supervision stays on real tokens only, so
the thoughts are free computation the model may use for planning. Control:
`pause` mode — identical slots filled with a literal learnable BOT token
(Goyal pause tokens / Pfau filler tokens), one parallel pass. The contrast
isolates recurrent latent computation from mere extra compute positions.
Decode does the same rollout at eval. Implementation: `inject` arg on
GPT.forward; `thoughts`/`thought_mode` train keys; ids BOT/EOT appended
past the task vocab. Cells: hp_sg_thought_48k_const (m=4, ~5× step cost),
hp_sg_pause_48k_const (m=4, ~1× cost), frozen schedule, 6 seeds each.
Anchors: ntp 0/6, mtp8 6/6.
